...
step_count= {"up": 0      , "down": 0      , "left": 0     , "right": 0     }
...
def search_from(maze, start_row, start_col):
    ...
    # Anfangswert von step_count 
    print("step_count =", step_count)
    old_col_reached = False
    ...
    while maze.is_exit(start_row, start_col) == False:
        ...
        while maze.look_forward(start_row, start_col, heading) == BLANK\
          and maze.look_right(start_row, start_col, heading) == OBSTACLE:
            ...
            print("step_count =", step_count)
            maze.update_position(start_row, start_col)
            # Abbruch wenn alte column wieder erreicht
            if step_count["left"] == step_count["right"]:
                print("alte column erreicht")
                old_col_reached = True
                break

        # wenn alte column wieder erreicht
        if old_col_reached == True:
            # drehen um die alte Richtung wiederherzustellen
            heading = maze.turn_left(heading)
            turn_count += 1
            # solange vorne frei ist
            while maze.look_forward(start_row, start_col, heading) == BLANK:
